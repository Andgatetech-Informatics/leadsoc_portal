...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (!company) {
      console.warn(`Organization not found for user: ${user.organization}`);
      return res.status(400).json({
        status: false,
        message: "Invalid organization ID. Please check organization setup.",
      });
    }
...
const sendOtp = async (req, res) => {
  try {
    ...
    if (!organization) return res.status(404).json({ message: "Failed to get organization" })
    if (!user) return res.status(404).json({ message: "User not found" });
    ...
    //Sending Email
    const forgetPassHtml = ForgetPass
      .replace(/{{firstName}}/g, user.firstName)
      .replace(/{{otp}}/g, user.otp)
      .replace(/{{organization}}/g, organization.organization)
      .replace(/{{year}}/g, "2025");
...
const changeUserRole = async (req, res) => {
  ...
  try {
    ...
    const allowedRoles = ["admin", "superadmin", "hr", "delivery"];
